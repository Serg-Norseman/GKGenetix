...
namespace GKGenetix.Core.Reference
{
    public static class RefData
    {
        ...
        public static SortedList<int, double>[] cM_Map
        {
            get {
                if (cM_map == null) {
                    ...
                    using (var ms = Utilities.GUnzip2Stream(GKGenetix.Core.Properties.Resources.map_csv)) {
        ...
        public static char[] RSRS
        {
            get {
                if (rsrs == null)
                    rsrs = Encoding.ASCII.GetString(Utilities.GUnzip2Bytes(GKGenetix.Core.Properties.Resources.RSRS)).ToCharArray();
        ...
        public static Dictionary<string, string[]> YMap
        {
            get {
                if (ymap == null) {
                    ...
                    var stm = Utilities.GUnzip2Stream(GKGenetix.Core.Properties.Resources.ysnp_hg19);
        ...
        private static List<MtDNAMapItem> mtdna_map = null;

        public static List<MtDNAMapItem> MtDnaMap
        {
            get {
                if (mtdna_map == null) {
                    mtdna_map = new List<MtDNAMapItem>();

                    // Map Locus (0), Starting (1), Ending (2), bp Length (3), Shorthand (4), Description (5)
                    string csv = GKGenetix.Core.Properties.Resources.mtdna_map;
                    using (StreamReader reader = new StreamReader(new MemoryStream(Encoding.ASCII.GetBytes(csv)))) {
                        string line;
                        while ((line = reader.ReadLine()) != null) {
                            string[] data = line.Split(new char[] { ',' });
                            mtdna_map.Add(new MtDNAMapItem(data[0], data[1], data[2], data[3], data[4], data[5]));
                        }
                    }
                }
        ...
        public static ISOGGYTreeNode ISOGGYTree
        {
            get {
                if (isoggYTree == null) {
                    XDocument doc = XDocument.Parse(GKGenetix.Core.Properties.Resources.ytree);
        ...
        public static MtDNAPhylogenyNode MtTree
        {
            get {
                if (mtTree == null) {
                    XDocument doc = XDocument.Parse(GKGenetix.Core.Properties.Resources.mtDNAPhylogeny);
        ...
        public static List<string> SnpOnTree
        {
            get {
                if (snpOnTree == null) {
                    ...
                    snpOnTree.AddRange(GKGenetix.Core.Properties.Resources.snps_on_tree.Split(new char[] { ',' }));
                }
                return snpOnTree;
            }
        }
